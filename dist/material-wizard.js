		...
		function(timer, $document, stepsArray) {
			return {
				...
				controller: ['$scope', function($scope) {
					...
					$scope.backExit = false;
					var steps = $scope.steps = [];

					this.addStep = function(stepScope) {
						steps.push(stepScope);
						stepsArray.push(stepScope);
						stepScope.progresscircleStyle = {
							"background-color": $scope.btnProgressColor
						};
						stepScope.wizardBtnStyle = {
							"background-color": $scope.btnBgColor
						};
						if (steps.length === 1) {
					...
					$scope.next = function() {
						...
						if (!$scope.backExit || typeof steps[$scope.selectedIndex].onBackExit !== 'function'){
                            ...
                            returnFunction = steps[$scope.selectedIndex].onBackExit();
                        }
					...
					function setStepActive(stepScope, active) {
						...
						stepScope.wizardBtnStyle = {
							"background-color": active ? $scope.activeBtnBgColor : $scope.btnBgColor
						};
					}
				}],
